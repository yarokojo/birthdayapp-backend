...
app.put('/api/users/profile', verifyToken, (req, res) => {
  ...
  res.json({
    ...
    network: updatedUser.network || 'MTN',
...
app.get('/api/wallet/transactions', verifyToken, (req, res) => {
  ...
  res.json({ transactions: data.wallets[userId].transactions || [] });
});
...
app.get('/api/calendar/events/me', verifyToken, (req, res) => {
  const userId = req.userId;
  console.log(`📅 GET /api/calendar/events/me for user ${userId}`);
...
app.post('/api/calendar/events', verifyToken, (req, res) => {
  ...
  console.log(`📅 POST /api/calendar/events for user ${userId}: ${title}`);
...
app.put('/api/calendar/events/:id', verifyToken, (req, res) => {
  ...
  console.log(`📅 PUT /api/calendar/events/${id} for user ${userId}`);
...
app.delete('/api/calendar/events/:id', verifyToken, (req, res) => {
  ...
  console.log(`📅 DELETE /api/calendar/events/${id} for user ${userId}`);
...
app.put('/api/calendar/events/:id/reminder', verifyToken, (req, res) => {
  ...
  console.log(`📅 PUT /api/calendar/events/${id}/reminder for user ${userId}`);
...
app.post('/api/user/support/feedback', verifyToken, (req, res) => {
  ...
  // Store feedback (could save to a file or database)
...
app.get('/api/leaderboard', (req, res) => {
  ...
  const usersWithScore = data.users.map(user => {
    ...
    const giftSent = data.wallets[user.id]?.totalSent || 0;
    const likesReceived = data.postLikes.filter(l => {
      const post = data.posts.find(p => p.id === l.postId);
      return post && post.userId === user.id;
    }).length;
    
    const score = (postCount * 10) + (giftReceived * 2) + (giftSent * 1) + (likesReceived * 5);
    ...
    return {
      ...
      profileImage: user.profileImage || 'https://randomuser.me/api/portraits/men/1.jpg',
      score,
      wishesSent: data.wallets[user.id]?.totalSent || 0,
...
// ============ POST COMMENT ROUTE (WITH DUPLICATE DETECTION) ============
app.post("/api/posts/:id/comments", verifyToken, (req, res) => {
  const { id } = req.params;
  const { text } = req.body;
  const userId = req.userId;
  ...
  console.log(`📊 Post now has ${post.comments} comments`);
  console.log("========================================");
  ...
  if (post.userId !== userId) {
    addNotification(
      ...
      "comment",
      "💬 New Comment",
      `${user?.name || "Someone"} commented on your post: "${text.trim().substring(0, 30)}${text.trim().length > 30 ? '...' : ''}"`
    );
  }
